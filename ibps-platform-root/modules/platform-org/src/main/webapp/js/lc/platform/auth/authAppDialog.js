...
(function() {
	...
	var defaults = {
		title:'应用选择框',
		url:__ctx+'/platform/auth/authApp/dialog.htm',
		isSingle:false, //是否单选
		isObj:false 	//是否返回对象
	};
	 ...
	 * @returns
	 */
	AuthAppDialog= function(options){
		this.options = $.extend({}, defaults, options);
	};
	
	AuthAppDialog.prototype={
			/**
			 * 显示窗口
			 */
			show:function(){
					...
					area:['60%','80%'],
					 btn:[{
			                ...
			                action: function(dialog,index) {
			              	  var data = DialogUtil.getChildFrameWindow(index).authAppSelector.getData();
			              	  if($.isEmpty(data)){
			            ...
			            },
			            {
			                ...
			                action: function(dialog,index) {
			                	...
			                	$.each(data, function(i, content) {
			                		DialogUtil.getChildFrameWindow(index).authAppSelector.remove(content.appKey);
			                	});
			                	DialogUtil.getChildFrameWindow(index).authAppSelector.clearSelect();
			                	/*if(options.isObj){
				              		options.callback(data);
			                	}else{
			                		options.callback([], []);
