...
(function($, window, document, undefined) {
	...
	var pluginName = "main", menus = [], topMenus = [], me = null, defaults = {
		key : 'id',
		parentKey : 'parentId',
		childKey : 'children',
		levelKey : 'level',
...
/*			$("#fontsize").slider({
		...
		fixedIbps : function() {
			// 头部
			try {
				ibps.settings.navbar_fixed(false, false);
			} catch (e) {
			}
			// 主体内容
			try {
				ibps.settings.check('main-container', 'fixed');
			} catch (e) {
			}
			// 侧边栏
			try {
				ibps.settings.sidebar_fixed(null, true);
			} catch (e) {
			}
			// 侧边栏 收缩、展开
		 ...
		 */
		initHome : function() {
			var url = $('#homePage').val(), isLocal = $("#isLocal").val(), baseUrl = $(
					"#baseUrl").val();
		...
		handerInnerMessage : function() {
			...
			$('#innerMessage')
					.click(function() {
								$.post(
												__ctx
														+ '/platform/msg/innerMessage/msgList.htm',
												function(result) {
													var r = '';
													if (result.records > 0) {
														for (var i = 0; i < result.rows.length; i++) {
															...
															r += '<li >'
																	+ '<a  onclick="mainPlugin.showReadMsgDlg(\''
																	+ msg.id
																	+ '\')"><div class="clearfix">'
					...
					.click(
							function() {
								$
										.post(
												__ctx
														+ '/platform/mail/outMail/msgList.htm',
												function(result) {
													var r = '';
													if (result.records > 0) {
														for (var i = 0; i < result.rows.length; i++) {
		...
		badgeTooltips : function() {
			// tooltip in sidebar items
			$('.sidebar .nav-list .badge[title],.navbar-menu .badge[title]')
					.each(
							function() {
								var tooltip_class = $(this).attr('class')
										.match(/tooltip\-(?:\w+)/);
								tooltip_class = tooltip_class ? tooltip_class[0]
										: 'tooltip-error';
								$(this)
										.tooltip(
												{
		...
		initMenu : function() {
			...
			$
					.post(
							__ctx + "/platform/auth/resources/getMenuData.htm",
							function(result) {
								menus = me.transformToTreeFormat(result);
								topMenus = me.getTopMenus(menus);
								var len = topMenus.length, menuItemHtml = '', top = null;
								// 添加顶部菜单
								for (var i = 0; i < len; i++) {
									top = topMenus[i];
									menuItemHtml += me.getTopMenuItem(top, i);
								}

								if (len >= 3) {
									menuItemHtml += me.getTopMoreMenuItem();
								}
								...
								var selectTab = $.cookie("selectTab") ? $
										.cookie("selectTab") : topMenus[0].id; // 如果cookie没有则获取第一项做为选中
								me.initSideMenu(selectTab);

								
							});

			// 顶部菜单监听
			menuContainer.delegate("li[role='presentation']", "click",
					function() {
						var self = $(this).find("a.menuItem"), id = self
								.attr("id");
						if ($.isEmpty(id))
							return;
						me.initSideMenu(id.split("_")[1], self);
					});
		},
		 ...
		 * 构建顶部
		 */
		getTopMenuItem : function(node, at) {
			var str = '<li role="presentation"> <a class="menuItem" href="javascript:void(0)" data-at="'
					+ at
					+ '"  data-toggle="tooltip"   position= "bottom" id="menu_'
					+ node.id + '" title="' + node.name + '">';
			if (node.icon != "null" && node.icon != "") {
				str += '<i class="ibps-icon bigger-110 fa ' + node.icon
						+ '"></i>';
			}

			var badge = $.isEmpty(node.desc) ? ''
					: ('<span class="badge badge-transparent tooltip-error" title="'
							+ node.desc
							+ '">'
							+ '<i class="ibps-icon fa fa-exclamation-circle  bigger-130"></i>' + '</span>');
			str += '<span class="menutext">' + node.name + badge + '</span>';
			str += '</a></li>';

			return str;
		},
		getTopMoreMenuItem : function() {
			var str = '<li class="dropdown" id="nav-sub-menu" tilte="更多">'
					+ '<a class="dropdown-toggle" data-toggle="dropdown"  href="javascript:void(0)" role="button">'
					+ '<i class="ibps-icon  bigger-110  fa fa-ellipsis-v " style="padding-top: 5px;"></i>'
					+ '</a>'
					+ '<ul class="dropdown-menu" role="menu" id="nav-sub-menu-ul">'
					+ '</ul>' + '</li>';
			return str;
		},
		gblen : function(text) {
		...
		initMoreMenu : function() {
			var navWidth = me.getNavWidth() ? me.getNavWidth() : null;
			$("#nav-sub-menu-ul").empty();
			if ($.isEmpty(navWidth)) {
				$("#nav-sub-menu").addClass("hidden");
				$("#nav-menu li[role='presentation']").show();
				return;
			}

			var len = topMenus.length, top, item = 0, width = 21;// 左右2边的边框
			for (var i = 0; i < len; i++) {
				top = topMenus[i];
				width += 7 * me.gblen(top.name) + 14 + 30; // 一个字符7px+字体图标+左右的边框
				if (width + 35 >= navWidth && !item) // 加个更多图标
					item = i;
			}
		
			if (width >= navWidth) {// 更多页面
				var li = $("#nav-menu li[role='presentation']"), lis = "";
				...
				$("#nav-sub-menu-ul").append(lis);
				$("#nav-sub-menu").removeClass("hidden");
			} else {
				var li = $("#nav-menu li[role='presentation']"), lis = "";
				li.each(function(i, n) {
					$(this).show();
		...
		getNavWidth : function() {
			...
			//宽度-LOGO标题-右边按钮-左右pading，
			return width - $("#navbar-header").outerWidth()
					- $("#navbar-buttons").outerWidth()-12*2;
		},
		/**
		 * 获取顶部按钮
		 * 
		...
		initSideMenu : function(id, menuId) {
			var nodes = {}, nameKey = this._defaults.nameKey, iconKey = this._defaults.iconKey, urlKey = this._defaults.urlKey, leftMenu = $("#leftMenu");

			nodes = me.getChildByParentId(id);
			leftMenu.empty();

			if (nodes && nodes.length) { // 子
				for (var i = 0; i < nodes.length; i++) {
					var s = this.getSideMenu(nodes[i]);
					leftMenu.append(s);
				}
				...
				var o = me.getTopMenu(id);
				if($.isEmpty(o)){
					id = topMenus[0].id;
					return  this.initSideMenu(id);
				}
				me.addTab({
					title : o[nameKey],
					url : o[urlKey],
			...
			if (!menuId)
				menuId = $("#menu_" + id);
			menuId.parent().siblings().removeClass("selected").end();
		...
		getSideMenu : function(o) {
			var me = this, topLevel = 1, levelKey = this._defaults.levelKey, isRoot = (o[levelKey] == topLevel ? true
					: false), m = isRoot ? me.getMenu(o)
					: ('<ul class="submenu">');
		...
		getMenu : function(n) {
			var me = this, childKey = this._defaults.childKey, levelKey = this._defaults.levelKey, nameKey = this._defaults.nameKey, iconKey = this._defaults.iconKey, urlKey = this._defaults.urlKey, isOpenKey = this._defaults.isOpenKey, aliasKey = this._defaults.aliasKey, descKey = this._defaults.descKey, topLevel = 1;

			var isHashChild = n[childKey] && n[childKey].length > 0 ? true: false, 
					arr = isHashChild ? '<span class="arrow fa fa-angle-right "></span>': '', 
					arr1 = isHashChild ? '<b class="arrow"></b>' : '',
					addTab = isHashChild ?( 'dropdown-toggle'+( n[urlKey]?' J_menuItem':'')): 'J_menuItem',
					options = 'data-id="' + n[aliasKey] + '" '
					+ 'data-title="' + n[nameKey] + '" ' + 'data-icon="'
					+ n[iconKey] + '" '
					+ (n[urlKey] ? ('data-url="' + n[urlKey] + '" ') : ''), 
					subMenu = isHashChild ? me.getSideMenu(n[childKey]): true, 
					icon = n[levelKey] == topLevel ? "menu-icon" : '';

			var badge = $.isEmpty(n[descKey]) ? ''
					: ('<span class="badge badge-transparent tooltip-error" title="'
							+ n[descKey]
							+ '">'
							+ '<i class="ibps-icon fa fa-exclamation-circle red bigger-130"></i>' + '</span>');

			li = '<li class="hover">' + '<a href="javascript:void(0)" class="'
					+ addTab + '" ' + options + '   >' + '<i class="' + icon
					+ ' fa ' + n[iconKey]
					+ '"></i>&nbsp;&nbsp;<span class="menu-text"  title="'
					+ n[nameKey] + '">' + n[nameKey] + badge + ' </span>' + arr
					+ '</a>' + arr1;
					...
					.contextmenu(
							{
								...
								before : function(context, e) {
									...
									if (id == "home") {// 禁用关闭
										$(
												'#context-menu a[data-action="tab-close-left"]')
												.addClass("disabled");
										$(
												'#context-menu a[data-action="tab-close"]')
												.addClass("disabled");
									} else {
										$(
												'#context-menu a[data-action="tab-close-left"]')
		...
		switchSkin : function(obj) {
			var me = $(obj), skin_class = me.attr('skin')
			me.parent().find('li').removeClass('disabled').removeClass(
					"grey-bg");
			...
			$('.ibps-nav > li:not(:last-child)').removeClass('light-pink').find(
					'> a > ' + ibps.vars['.icon']).removeClass('pink').end().eq(
					0).find('.badge').removeClass('badge-warning');
			$('.sidebar-shortcuts .btn').removeClass('btn-pink btn-white')
					.find(ibps.vars['.icon']).removeClass('white');

			// undo skin-3
			$('.ibps-nav > li.grey').removeClass('red').find('.badge')
			...
			var skin0_buttons = [ 'btn-success', 'btn-info', 'btn-warning',
					'btn-danger' ];
			if (skin_class == 'no-skin') {
				...
				$('.ibps-nav > li:not(:last-child)').addClass('light-pink')
						.find('> a > ' + ibps.vars['.icon']).addClass('pink')
						.end().eq(0).find('.badge').addClass('badge-warning');

				$('.sidebar-shortcuts .btn').attr('class',
						'btn btn-white btn-pink').find(ibps.vars['.icon'])
						.addClass('white');
			}

			// skin-3
			// change shortcut buttons classes, this should be hard-coded if you
			// want to choose this skin
			else if (skin_class == 'skin-3') {
				body.addClass('no-skin');// because skin-3 has many parts of
											// no-skin as well

				$('.ibps-nav > li.grey').addClass('red').find('.badge')
						.addClass('badge-yellow');

				var i = 0;
				$('.sidebar-shortcuts .btn').each(
						function() {
							$(this).attr('class', 'btn btn-primary btn-white');
							$(this).find(ibps.vars['.icon']).addClass(
									skin3_colors[i++]);
						})
		...
		switchSystem : function(systemId) {
			window.location.href = __ctx
					+ "/platform/console/saveCurrSys.htm?systemId=" + systemId;
		},

		// 修复iFrame高度
		flexFrameHeight : function() {
			var $height = me.getWidthHeight().height, headerHeight = $(
					'#navbar').height(), tabHeight = $('.content-tabs')
					.height();

			$height = $height - headerHeight - tabHeight-10;
			...
			setTimeout(
					function() {
						var windowH = $(window).height(), 
							...
							sideH = windowH- headerH, 
							contH = windowH - headerH - navH;
		...
		switchUser : function(account) {
			if('shiro' == __auth){
				$.post(__ctx + "/runAs.htm",
					{j_username : account}, function(data) {
						if (data.result == 1) {
							window.location.href = __ctx + "/platform/console/main.htm";
						} else {
							laydialog.msg(data.message);
		...
		showVerInfo:function(){
			DialogUtil.dialog({
				title : "版本信息",
				area: ['30%', '40%'],
				content : __ctx + '/platform/console/verInfo.htm',
				btn:[{
	            	label: '确定',
	            	iconCls:'btn btn-primary fa fa-comfirm',
