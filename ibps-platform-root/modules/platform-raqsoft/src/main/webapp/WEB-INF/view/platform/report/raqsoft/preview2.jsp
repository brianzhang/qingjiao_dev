...
<html>
	...
	<body>
		<div id="report_preview" class="wrapper wrapper-content  animated fadeInRight col-sm-12">
					...
					<div class="toolbar-head clearfix">
					 ...
					 <div class="btn-group">
							<!-- 	<a class="btn btn-primary fa fa-search" href="javascript:void(0);" ><span>搜索</span></a>
							 ...
							 -->
								<a href="#" onClick="javascript:history.go(-1);" class="btn btn-primary fa fa-back" ><span>返回</span></a>
							</div> 
			...
			<!--	<iframe id="raqsoftFrame" src="${ctx }/platform/report/raqsoft/showReport.htm?reportName=${reportName}&${cname1}=${cval1}" style="width:100%;border: 0px;"></iframe>  -->
				<iframe id="raqsoftFrame" src="${ctx }/platform/report/raqsoft/showReport.htm?reportId=${reportId}&${cname1}=${cval1}" style="width:100%;border: 0px;" ></iframe>  
...
{{each reportParams as item i}}
	...
	<label class="search-label" style="visibility:hidden" >{{item.desc}}</label>
	{{if item.controlType == undefined||item.controlType == 'text'||item.controlType == ''}}<input type="text"  style="visibility:hidden" name="{{item.name }}"  class="form-control"  value="{{item.value }}" />{{/if}}
	{{if item.controlType == 'datePicker'}}
		<input type="text" style="visibility:hidden"  name="{{item.name }}"  class="form-control {{if item.controlOptions.datefmt_type=='date'}}date{{/if}}{{if item.controlOptions.datefmt_type=='time'}}time{{/if}}{{if item.controlOptions.datefmt_type=='datetime'}}datetime{{/if}}"  value="{{item.value }}" />
	{{/if}}
...
<script type="text/html" id="s:showReport:print">
	<div class="btn-group">
		<a class="btn btn-primary fa fa-print" href="#"><span>打印</span></a>
	</div>
	<div class="btn-group">
		<a class="btn btn-primary fa fa-file-pdf-o fa-file-pdf-e" href="#" ><span>PDF导出</span></a>
		<a class="btn btn-primary fa fa-file-excel-o fa-file-excel-e" href="#" ><span>EXCEL导出</span></a>
		<a class="btn btn-primary fa fa-file-word-o fa-file-word-e" href="#" ><span>WORD导出</span></a>
	</div>
</script>
	</body>
