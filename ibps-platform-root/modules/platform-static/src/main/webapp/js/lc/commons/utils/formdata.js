	...
	$.extend($.fn, {
		...
		sortList:function(){
			...
			form.find('[type="subtable"]').each(function(){
				...
				var index=0;
				subTable.find('[type="append"]').remove();
				subTable.find('[type="subdatadiv"]').each(function(){
					$(this).find("input,select,textarea").each(function(){
						var name=$(this).attr('name');
						var sortName=tableId+'List'+'['+index+'].'+name;
						$(this).attr('name',sortName);
		...
		setData:function(){
			...
			var main={};
			mainTable.find('input:text,input:hidden,textarea,select,input:password').each(function(){
				var value=$(this).val();
				var name=$(this).attr('name');
				if(value!=null&&value!=''){
					main[name]=value;
			...
			form.find('[type="sub"]').each(function(){
				 ...
				 subTable.find('[type="subdata"]').each(function(){
					 ...
					 //设定checkbox，radio的值。
					 self.getCheckBoxValue(subJson, $(this));
