...
QueryBuilder.selectors = {
    ...
    control_container:   '.rule-control-container',
    source_container:   '.rule-source-container',
    ...
    rule_control:        '.rule-control-container [name$=_control]',
    rule_source:        '.rule-source-container [name$=_source]',
...
QueryBuilder.prototype.bindEvents = function() {
    ...
    // rule source change
    this.$el.on('change.queryBuilder', Selectors.rule_source, function() {
    	var $rule = $(this).closest(Selectors.rule_container);
    	self.getModel($rule).source = $(this).val();
    });
    
    // rule control change
    this.$el.on('change.queryBuilder', Selectors.rule_control, function() {
        var $rule = $(this).closest(Selectors.rule_container);
        self.getModel($rule).control = $(this).val();
    });
        ...
        'update': function(e, node, field, value, oldValue) {
            if (node instanceof Rule) {
                switch (field) {
                    ...
                    case 'control':
                    	self.updateRuleControl(node, oldValue);
                    	break;
                    	
                    case 'source':
                    	self.updateRuleSource(node, oldValue);
                    	break;
...
/**
 * Creates the sources for a rule and init the rule source
 * @param {Rule} rule
 * @fires QueryBuilder.afterCreateRuleSources
 * @private
 */
QueryBuilder.prototype.createRuleSources = function(rule) {
    var $sourceContainer = rule.$el.find(QueryBuilder.selectors.source_container).empty();

    if (!rule.filter) {
        return;
    }

    var sources = this.getSources(rule.filter);
    var $sourceSelect = $(this.getRuleSourceSelect(rule, sources));
    $sourceContainer.html($sourceSelect);

    // set the source without triggering update event
    rule.__.source = sources[0];

    /**
     * After creating the dropdown for sources
     * @event afterCreateRuleSources
     * @memberof QueryBuilder
     * @param {Rule} rule
     * @param {QueryBuilder.Source[]} sources - allowed sources for this rule
     */
    this.trigger('afterCreateRuleSources', rule, sources);
};

/**
 * Creates the controls for a rule and init the rule control
 * @param {Rule} rule
 * @fires QueryBuilder.afterCreateRuleControls
 * @private
 */
QueryBuilder.prototype.createRuleControls = function(rule) {
    var $controlContainer = rule.$el.find(QueryBuilder.selectors.control_container).empty();

    if (!rule.filter) {
        return;
    }

    var controls = this.getControls(rule.filter);
    var $controlSelect = $(this.getRuleControlSelect(rule, controls));
    $controlContainer.html($controlSelect);

    // set the control without triggering update event
    rule.__.control = controls[0];

    /**
     * After creating the dropdown for controls
     * @event afterCreateRuleControls
     * @memberof QueryBuilder
     * @param {Rule} rule
     * @param {QueryBuilder.Control[]} controls - allowed controls for this rule
     */
    this.trigger('afterCreateRuleControls', rule, controls);
};
...
QueryBuilder.prototype.createRuleInput = function(rule) {
    ...
    if(rule.source == 'script'){
    	filter.input = 'textarea';
    }else if(rule.source == 'fixed'){
    	filter.previous_input = filter.previous_input?filter.previous_input: filter.input;
    	filter.input =  filter.previous_input?filter.previous_input:filter.input;
    }
...
QueryBuilder.prototype.updateRuleFilter = function(rule, previousFilter) {
    ...
    this.createRuleSources(rule);
    this.createRuleInput(rule);
    
    var $controlContainer = rule.$el.find(QueryBuilder.selectors.control_container);
...
QueryBuilder.prototype.updateRuleOperator = function(rule, previousOperator) {
	var $sourceContainer = rule.$el.find(QueryBuilder.selectors.source_container);
    ...
    if (!rule.operator || rule.operator.nb_inputs === 0) {
        ...
        if ($sourceContainer.is(':empty') || !previousOperator ||
...
/**
 * Performs actions when a rule's source changes
 * @param {Rule} rule
 * @param {object} ruleValue
 * @fires QueryBuilder.afterUpdateRuleSource
 * @private
 */
QueryBuilder.prototype.updateRuleSource = function(rule, previousSource) {
	var $controlContainer = rule.$el.find(QueryBuilder.selectors.control_container);
    var $valueContainer = rule.$el.find(QueryBuilder.selectors.value_container);
	 var ruleValue = rule.value;
    var __source = rule.source;
    if (!rule.operator || rule.operator.nb_inputs === 0) {
    	$controlContainer.hide();
    	$valueContainer.hide();
        ...
        switch(__source){
        case 'dynamic':
        	if ($controlContainer.is(':empty')) {
                this.createRuleControls(rule);
            }
        	rule.control = rule.filter.origin_field.field_type;
    ...
    if (rule.source) {
        rule.$el.find(QueryBuilder.selectors.rule_source).val(rule.source);
    }

    /**
     *  After the source has been updated and the input optionally re-created
     * @event afterUpdateRuleSource
     * @memberof QueryBuilder
     * @param {Rule} rule
     */
    this.trigger('afterUpdateRuleSource', rule);
...
/**
 * Performs actions when a rule's control changes
 * @param {Rule} rule
 * @param {object} previousControl
 * @fires QueryBuilder.afterUpdateRuleControl
 * @private
 */
QueryBuilder.prototype.updateRuleControl = function(rule, previousControl) {
    if (rule.control) {
        rule.$el.find(QueryBuilder.selectors.rule_control).val(rule.control);
    }

    /**
     *  After the control has been updated and the input optionally re-created
     * @event afterUpdateRuleControl
     * @memberof QueryBuilder
     * @param {Rule} rule
     */
    this.trigger('afterUpdateRuleControl', rule);
};
...
QueryBuilder.prototype.applyRuleFlags = function(rule) {
    ...
    if (flags.control_readonly) {
    	rule.$el.find(Selectors.rule_control).prop('disabled', true);
    }
    if (flags.source_readonly) {
    	rule.$el.find(Selectors.rule_source).prop('disabled', true);
    }
...
QueryBuilder.prototype.validate = function(options) {
    ...
    var valid = (function parse(group) {
        ...
        group.each(function(rule) {
            ...
            //如果值来源不验证
            if (rule.source !=='dynamic'  && rule.operator.nb_inputs !== 0) {
    ...
    var out = (function parse(group) {
        ...
        group.each(function(rule) {
            ...
            var ruleData = {
                ...
                control: rule.control ? rule.control : null,
...
/**
 * Returns the controls for a filter
 * @param {string|object} filter - filter id or filter object
 * @returns {object[]}
 * @fires QueryBuilder.changer:getControls
 * @private
 */
QueryBuilder.prototype.getControls = function(filter) {
    if (typeof filter == 'string') {
        filter = this.getFilterById(filter);
    }
    ...
    for (var i = 0, l = this.controls.length; i < l; i++) {
        ...
        if (filter.controls) {
            if (filter.controls.indexOf(this.controls[i]) == -1) {
                continue;
            }
        }
    ...
    /**
     * Modifies the controls available for a filter
     * @event changer:getControls
     * @memberof QueryBuilder
     * @param {QueryBuilder.Control[]} controls
     * @param {QueryBuilder.Filter} filter
     * @returns {QueryBuilder.Control[]}
     */
    return this.change('getControls', result, filter);
};

/**
 * Returns the sources for a filter
 * @param {string|object} filter - filter id or filter object
 * @returns {object[]}
 * @fires QueryBuilder.changer:getSources
 * @private
 */
QueryBuilder.prototype.getSources = function(filter) {
    if (typeof filter == 'string') {
        filter = this.getFilterById(filter);
    }
    ...
    for (var i = 0, l = this.sources.length; i < l; i++) {
        ...
        if (filter.sources) {
            if (filter.sources.indexOf(this.sources[i]) == -1) {
                continue;
            }
        }
    ...
    /**
     * Modifies the sources available for a filter
     * @event changer:getSources
     * @memberof QueryBuilder
     * @param {QueryBuilder.Source[]} sources
     * @param {QueryBuilder.Filter} filter
     * @returns {QueryBuilder.Sources[]}
     */
    return this.change('getSources', result, filter);
};
...
<li id="{{= it.rule_id }}" class="rule-container"> \
  ...
  <div class="rule-source-container"></div> \
  <div class="rule-control-container"></div> \
...
QueryBuilder.templates.controlSelect = '\
	{{? it.controls.length === 1 }} \
	<span> \
	{{= it.translate("controls", it.controls[0]) }} \
	</span> \
	{{?}} \
	<select class="form-control {{? it.controls.length === 1 }}hide{{?}}" name="{{= it.rule.id }}_control"> \
	  {{~ it.controls: control }} \
	    <option value="{{= control }}">{{= it.translate("controls", control) }}</option> \
	  {{~}} \
	</select>';

QueryBuilder.templates.sourceSelect = '\
	{{? it.sources.length === 1 }} \
	<span> \
	{{= it.translate("sources", it.sources[0]) }} \
	</span> \
	{{?}} \
	<select class="form-control {{? it.sources.length === 1 }}hide{{?}}" name="{{= it.rule.id }}_source"> \
	  {{~ it.sources: source }} \
	    <option value="{{= source }}">{{= it.translate("sources", source) }}</option> \
	  {{~}} \
	</select>';
...
/**
 * Returns rule's control HTML
 * @param {Rule} rule
 * @param {object[]} controls
 * @returns {string}
 * @fires QueryBuilder.changer:getRuleControlTemplate
 * @private
 */
QueryBuilder.prototype.getRuleControlSelect = function(rule, controls) {
    var h = this.templates.controlSelect({
        builder: this,
        rule: rule,
        controls: controls,
        icons: this.icons,
    ...
    /**
     * Modifies the raw HTML of the rule's control dropdown
     * @event changer:getRuleControlTemplate
     * @memberof QueryBuilder
     * @param {string} html
     * @param {Rule} rule
     * @param {QueryBuilder.Control[]} controls
     * @returns {string}
     */
    return this.change('getRuleControlSelect', h, rule, controls);
};
 ...
 * @returns {string}
 * @fires QueryBuilder.changer:getRuleSourceTemplate
 * @private
 */
QueryBuilder.prototype.getRuleSourceSelect = function(rule, sources) {
    var h = this.templates.sourceSelect({
        builder: this,
        rule: rule,
        sources: sources,
        icons: this.icons,
    ...
    /**
     * Modifies the raw HTML of the rule's source dropdown
     * @event changer:getRuleSourceTemplate
     * @memberof QueryBuilder
     * @param {string} html
     * @param {Rule} rule
     * @param {QueryBuilder.Source[]} sources
     * @returns {string}
     */
    return this.change('getRuleSourceSelect', h, rule, sources);
};
...
var Rule = function(parent, $el) {
    ...
    /**
     * @name control
     * @member {QueryBuilder.Control}
     * @memberof Rule
     * @instance
     */
    this.__.control = null;
...
Utils.defineModelProperties(Rule, ['filter', 'operator', 'control', 'source','value']);
...
QueryBuilder.define('bt-selectpicker', function(options) {
    ...
    this.on('afterUpdateRuleSource', function(e, rule) {
    	rule.$el.find(Selectors.rule_source).selectpicker('render');
    });
    
    this.on('afterUpdateRuleControl', function(e, rule) {
    	rule.$el.find(Selectors.rule_control).selectpicker('render');
    });

    this.on('beforeDeleteRule', function(e, rule) {
        rule.$el.find(Selectors.rule_filter).selectpicker('destroy');
        rule.$el.find(Selectors.rule_operator).selectpicker('destroy');
        rule.$el.find(Selectors.rule_source).selectpicker('destroy');
        rule.$el.find(Selectors.rule_control).selectpicker('destroy');
    });
}, {
    container: 'body',
