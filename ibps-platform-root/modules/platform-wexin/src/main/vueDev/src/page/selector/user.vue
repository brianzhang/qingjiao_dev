  ...
  methods: {
    load: function(obj) {
      ...
      if (obj) {
        ...
        if(that.type=='role'){
          url = that.roleUrl + "?" +qs.stringify({subSysId:obj.id});
        }
      }
    ...
    findPerson:function(parentId,that){
      ...
      var parameter = {partyType:that.type,partyId:parentId};
      http.get(
    ...
    changeUrl: function(that){
      switch (that.type) {
        case 'org':
          ...
          that.navigate[0].url = webapiUrl + "/webapi/orgService/findByParentId?parentId=0"
          break;
        case 'position':
          that.url = webapiUrl + "/webapi/positionService/findByParentId";
          that.navigate[0].url = webapiUrl + "/webapi/positionService/findByParentId?parentId=0"
          break;
        case 'role':
          that.url = webapiUrl + "/webapi/subSysService/getAllSubSys";
          that.navigate[0].url = webapiUrl + "/webapi/subSysService/getAllSubSys";
          break;
      }
    }
  },
