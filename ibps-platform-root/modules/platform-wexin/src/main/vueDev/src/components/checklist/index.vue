<template>
  <div>
    <div :id="id_">
      <checklist :title="title" label-position="right" :check-disabled=true :options="getOptionsList()" v-model="checkedList" :max="max" :min="min" ></checklist>
    </div>
  </div>
</template>
    ...
    mounted(){
      if(this.isDisabled){
        ...
        for(var i=0;i<va.length;i++){
          va[i].setAttribute("disabled", "true");
        }
      }
    },
    computed : {
