<template>
  <div>
    <div class="header">发布人</div>
    <div style="height:.5rem;line-height:.5rem;font-size: .18rem;">
...
export default {
  ...
  methods: {
    ...
    findPerson:function(parentId,that){
      ...
      var parameter = {partyType:that.type,partyId:parentId,account:that.account};
      http.get(
    ...
    changeUrl: function(that){
      switch (that.type) {
        case 'org':
          ...
          that.navigate[0].url = webapiUrl + "/webapi/orgService/findByParentId?parentId=0&account="+that.account;
          break;
        case 'position':
          that.url = webapiUrl + "/webapi/positionService/findByParentId";
          that.navigate[0].url = webapiUrl + "/webapi/positionService/findByParentId?parentId=0&account="+that.account;
          break;
        case 'role':
          that.url = webapiUrl + "/webapi/subSysService/getAllSubSys";
          that.navigate[0].url = webapiUrl + "/webapi/subSysService/getAllSubSys&account="+that.account;
          break;
      }
    },
