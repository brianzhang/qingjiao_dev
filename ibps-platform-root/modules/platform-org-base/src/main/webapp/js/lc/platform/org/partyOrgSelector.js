 ...
 * </pre>
 */
var partyOrgSelector;
var single = window.isSingleSelector, groupsTree=null;
$(function() {
	partyOrgSelector = new PartyOrgSelector();
	partyOrgSelector.init();
});
...
(function() {
	var template = '<span class="attach-span" orgid="#orgid#" orgname="#orgname#" orgkey="#orgkey#">' +
								...
								'&nbsp;' +
								'<a class="btn btn-ms" title="移除" href="javascript:partyOrgSelector.remove(\'#orgid#\',\'#orgkey#\');"><i class="fa fa-remove"></i></a>' +
								'</span>'+
							'</span>';
	 ...
	 * 组织选择框 对象
	 * 
	 * @returns {PartyOrgSelector}
	 */
	PartyOrgSelector = function() {
	...
	PartyOrgSelector.prototype = {
		...
		remove:function(id,key){
			var obj = null;
			if(id && "" != id) obj = $("span[orgid='"+id+"']");
			if(key && "" != key) obj = $("span[orgkey='"+key+"']");
		...
		clearSelect:function(){
			...
			var nodes = me.partyOrgTree.getSelectedNodes();
			if(!nodes || nodes.length < 1) return;
			for(var i = 0; i < nodes.length; i ++){
				me.partyOrgTree.checkNode(nodes[i], false, true);
			}
		},
		/**
		 * 初始数据
		...
		getData:function(){
			...
			$.each(data, function(i, n){
				var obj = {id:$(n).attr("orgid"), name:$(n).attr("orgname"), key:$(n).attr("orgkey")};
				aryData.push(obj);
			});
			return aryData;
		},
