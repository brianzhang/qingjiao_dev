...
(function() {
	...
	var defaults = {
			title:'参与者选择器',
			url:__ctx+'/platform/org/partyDialog/party.htm',
			isSingle:false, //是否单选
			isObj:false ,//是否返回对象
	 ...
	 * @returns
	 */
	PartyDialog= function(options){
		this.options = $.extend({}, defaults, options);
	};
	
	PartyDialog.prototype={
			/**
			 * 显示窗口
			 */
			show:function(){
					...
					area:['66%','88%'],
					 btn:[{
			                ...
			                action: function(dialog,index) {
			              	  var  data = DialogUtil.getChildFrameWindow(index).partyDialogParty.getData();
			              	  if($.isEmpty(data)){
			              	  ...
			              	  if(options.isObj){
			              		...
			              		var entityIds=[],entityNames=[];
			          			$.each(data, function(i, content) {
									var entityid=content.entityid;
									var entityname=content.entityname;
									entityIds.push(entityid);
									entityNames.push(entityname);
								});
			          			options.callback(entityIds,entityNames);
			              	  }
			            ...
			            },
			            {
			                ...
			                action: function(dialog,index) {
			                	...
			                	$.each(data, function(i, content) {
		                			DialogUtil.getChildFrameWindow(index).partyDialogParty.remove(content.entityid,content.entitykey);
			                	});
			                	if(options.isObj){
				              		options.callback(data);
			                	}else{
