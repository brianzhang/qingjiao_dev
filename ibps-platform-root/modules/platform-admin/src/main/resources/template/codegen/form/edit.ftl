...
********************************************
-->
<#macro field_control field classVars isMain showVal idx>
		...
		<#assign val><#if showVal><#if field.dataType=='date'><fmt:formatDate value="<#noparse>${</#noparse>${classVars}.${field.name}}"  pattern="${field.field_options.datefmt}"/><#else><#noparse>${</#noparse>${classVars}.${field.name}}</#if></#if></#assign>
		<#switch field.field_type>
			...
			<#case "radio"><#---单项选择-->
				<#list  field.field_options.options as option>
					<label class="fr-control-option radio-inline">
					    <input type="radio" name="${controlName}" value="${option.val}" defaultVal="${option.checked}" class="ibps" <#if showVal><c:if test="<#noparse>${</#noparse>${classVars}.${field.name}==null}">defaultValue="true"</c:if></#if> <#if showVal><c:if test="<#noparse>${</#noparse>${classVars}.${field.name}=='${option.val}'}">checked="checked"</c:if></#if>  validate="{<#if field.field_options.required>required:true<#else>required:false</#if>}"/>
			...
			<#case "checkbox"><#---多项选择-->
				<#list  field.field_options.options as option>
					<label class="fr-control-option checkbox-inline">
					    <input type="checkbox" name="${controlName}" class="ibps" value="${option.val}" <#if showVal><c:if test="<#noparse>$</#noparse>{fn:contains(${classVars}.${field.name}, '${option.val}')}">checked="checked"</c:if></#if> validate="{<#if field.field_options.required>required:true<#else>required:false</#if>}"/>
					   	<span class="lbl">${option.label}</span>
				  </label>
			  </#list>
			<#break>
			<#case "select"><#---下拉框-->
				<select class="fr-form-control" name="${controlName}"  value="<#noparse>${</#noparse>${classVars}.${field.name}}" validate="{<#if field.field_options.required>required:true<#else>required:false</#if>}">
					 <#if field.field_options.include_blank_option>
					 <option value="">请选择</option>
					 </#if>
				  	<#list  field.field_options.options as option>
				    <option value="${option.val}" <#if showVal><c:if test="<#noparse>${</#noparse>${classVars}.${field.name}=='${option.val}'}">selected="selected"</c:if></#if>>${option.label}</option>
				    </#list>
				</select>
			<#break>
			...
			<#case "dictionary"><#---数据字典-->
				...
				<input type="text" readonly="readonly"  class="fr-form-control comboTree"
		                 ...
		                 <#if showVal>value="<#noparse>${</#noparse>f:getDictLabel2(${classVars}.${field.name},'${field.field_options.dictionary}', 'key','${field.field_options.display_mode}','${field.field_options.split}', '')}"</#if>  validate="{<#if field.field_options.required>required:true<#else>required:false</#if>}"/>
			<#break>
			...
			<#case "customDialog"><#---自定义对话框-->
				<div class="input-group" data-toggle="customdialog"  data-type="${field.field_options.dialog_type}" data-dialog="${field.field_options.dialog}" data-bind="${listToString(field.field_options.bind)}" data-name="${keyTag}:${classVars}:">
					<input type="text" readonly="readonly"  class="fr-form-control "  name="${controlName}"  <#if showVal>value="${val}"</#if> validate="{<#if field.field_options.required>required:true<#else>required:false</#if>}"/>
...
<#macro sub_table_control field classVars subClassVars isMain>
		...
		<table name="${tblName}" data-mode="${mode}" data-required="${field.field_options.required}" class="table table-bordered table-hover table-condensed">
			...
			<#list  field.field_options.columns as column>
				 	 	 <#if column.field_type == 'hidden' || column.field_options.hide ><@field_control  field=field  classVars=classVars isMain=isMain showVal=true idx='stat'/><#else>
				 		<td>
						<#if mode=='inner'>	<#-- 行内编辑模式 -->
							<@field_control  field=column  classVars=subClassVars isMain=isMain showVal=true idx='stat'/>
						<#elseif mode =='dialog'>	<#-- 弹窗模式 -->
							<#assign val><#if column.dataType=='date'><fmt:formatDate value="<#noparse>${</#noparse>${classVars}.${column.name}}"  pattern="${column.field_options.datefmt}"/><#else><#noparse>${</#noparse>${classVars}.${column.name}}</#if></#assign>
							<input type="hidden" name="${keyTag}:${subClassVars}:${column.name}:<#noparse>${</#noparse>stat.index+1}"  value="${val}"/><span>${val}</span>
						</#if>
						</td>	
						  </#if>	
				</#list>
	...
	<#if mode=='inner'> <#-- 行内编辑模式 -->
		<tr>
		  	<td><input role="checkbox" class="cbox " type="checkbox" name="${tblName}" ></td>
		 <#list  field.field_options.columns as column>
	 	 	 <#if column.field_type == 'hidden' || column.field_options.hide ><@field_control  field=field  classVars=classVars isMain=isMain showVal=false idx='idx'/><#else><td>
				<@field_control  field=column  classVars=subClassVars isMain=isMain showVal=false idx='idx'/>
			</td>
			</#if>	
		</#list>
		  	<td class="fr_table_col_remove" width="${width}">
	...
	<#elseif mode =='dialog'><#-- 弹窗模式 -->
		<tr>
		  	<td><input role="checkbox" class="cbox " type="checkbox" name="${tblName}" ></td>
		 <#list  field.field_options.columns as column>
	 	 	 <#if column.field_type == 'hidden' || column.field_options.hide ><input type="hidden" name="${tblName}:${column.name}:{{idx}}"  value="{{${column.name}}}"/><#else>
	 		<td>
...
<#macro response_field field tableName isMain>
	...
	<#else >
		 	 <#if field.field_type == 'hidden' ||  field.field_options.hide >
		 			<@field_control  field=field  classVars=classVars isMain=isMain showVal=true idx=0/>
			 <#else>
