...
<a href="javascript:void(0)"  data-event="action" data-url="${ctx}/platform/system/news/get.htm?id=${data.id}" class="item-name primary-link">${data.title}</a>
<span class="item-label"></span>
</div>
<span class="item-status">
<span class="badge badge-empty badge-success"></span>${data.userName}</span>
</div>
<div class="item-body">${data.publicDate?string("yyyy-MM-dd")}</div>
</div>
	</#list>
</div>
<#else>
