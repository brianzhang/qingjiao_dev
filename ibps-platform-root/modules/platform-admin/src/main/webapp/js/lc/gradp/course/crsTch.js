	...
	CrsTch.prototype = {
		...
		_initGridList : function() {
			...
			$(this.consts.GRID)
					.GridList(
							{
								url : __ctx
										+ '/gradp/course/crsTch/listJson.htm?curTerm='+curTerm,
											...
											formatoptions : [
													{
														...
														action : 'listMyStudents.htm?crsTchId={id}&crsName={crsName}',
													},{
														label : ' 课程作业管理',
		...
		_initForm : function() {
			...
			$(document).on('click','a.fa-save',function() {
							...
							var g = __ctx + '/gradp/course/crsTch/save.htm',
							h = JSON2.stringify(e);
							if(!admin){
								DialogUtil.confirm('是否同时清空当前<span style="color:red">所有课程学生作业及相关信息</span>？',
										function(d) {
											DialogUtil.alert("正在初始化数据，请稍候");
											$.post(g, {
												crsTchPo: h,
												reset : d,
												copy : copy
								...
								$.post(__ctx+'/gradp/course/crsTch/save.htm' , {admin:true , crsTchPo: h },function(b) {
										var c = new com.lc.form.ResultMessage(b);
										if (c.isSuccess()) {
											DialogUtil.msg(c.getMessage());
											window.location.href = preUrl;
		...
		edit : function(crsTchId,crsName) {
			DialogUtil.dialog({
				title : '作业管理',
				content : __ctx + '/gradp/course/crsJob/list.htm?crsTchId='
		...
		initTableSelect2 : function() {
			try {
				var me = this, $el = $('#param').length>0 ? $('#param') : $('#source'), options = $el.data(),cooc = options.cooc, $val = options.value, $comment = options.comment, ajax = options.ajax, multiple = $
