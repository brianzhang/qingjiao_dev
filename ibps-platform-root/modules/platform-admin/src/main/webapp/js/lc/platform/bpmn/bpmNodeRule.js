...
function updateContent(item,data){
	...
	if(ruleType=='1'){
		...
		var judge = $('<div class="judge left margin-set"></div>')
					.append($('<span id="conDesc">'+data.conDesc+'</span>'));
		item.data("data",data); 
		nobr.append(labelSpan).append(judge);
		item.append(nobr); 
		var editBtn =$('<a class="btn btn-primary fa fa-edit" onclick="eidtCondition(this)" style="float:right"></a>')
		item.append(editBtn);
	}
	...
	//脚本规则
		...
		var editBtn =$('<a class="btn btn-primary fa fa-edit" style="float:right" onclick="eidtCondition(this)"></a>')
		item.append(editBtn);
	}
	else {//组属性、用户属性 
		if($.isEmpty(paramType))paramType="用户";
		...
		/*if(ruleType=='3'){
			...
			labelSpan = $('<span class="label-span left"></span>').attr("ruleType",ruleType).text('组属性');
		}*/
...
function eidtCondition(div){
	...
	if(data.ruleType ==1){
		 new RuleConditionDialog({params:data,defId:defId,nodeId:nodeId,callback:function(data){
			item.data("data",data);
			$("#conDesc",item).text(data.conDesc);
		}}).show();
	}else{
		new ScriptDialog({params:data,defId:defId,nodeId:nodeId,callback:function(data){
			item.data("data",data);
			$("#conDesc",item).text(data.conDesc);
		}}).show();
	}
