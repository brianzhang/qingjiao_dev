 ...
 */
$(function() {
	formDataTemplateList  = new FormDataTemplateList();
	formDataTemplateList.init();
...
(function() {
	...
	/**
	 * 表单列表 对象
	 * @returns {formDataTemplateList}
	 */
	FormDataTemplateList = function() {
	 ...
	 * 方法
	 */
	FormDataTemplateList.prototype = {
		consts:	_consts,
		/**
		 * 初始化
		 */
		...
		handlerSearch : function() {
			...
			$('a.fa-import').click(function(){
				DialogUtil.dialog({
					title:'导入业务数据模版',
					area: ['30%', '40%'],
					content:__ctx + '/platform/form/formDataTemplate/importDialog.htm',
					btn:[{
		            	label: '导入',
		...
		loadFormList:function(params){
			...
			$.ajax({
		        url:  __ctx+ "/platform/form/formDataTemplate/listJson.htm",
		        data:params,
		        type:'POST',
		...
		bindSettingsOptions:function(_this){
			...
			$(document).off( "click", ".delete-form" ).on("click", ".delete-form", function(e) {
				e.preventDefault(); e.stopPropagation();
			 	 var el = $(this), option =  el.closest(".dashboard-settings-options"),
			 	 	url =  __ctx+"/platform/form/formDataTemplate/remove.htm?id="+option.data("id");
			...
			$(document).off( "click", ".preview-form" ).on("click", ".preview-form", function(e) {
				e.preventDefault(); e.stopPropagation();
				var el = $(this), option =  el.closest(".dashboard-settings-options"),
				url =  __ctx+"/platform/form/formDataTemplate/preview.htm?id="+option.data("id");
			    DialogUtil.dialog({
			    	content:url,
			    	area: ['100%', '100%'],
					maxmin:false,
			...
			$(document).off( "click", ".view-url" ).on("click", ".view-url", function(e) {
				...
				DialogUtil.alert('/f/'+id+'.htm','查看URL');
				$('.settings').webuiPopover('hide');
			});
		},
		...
		create:function(){
			...
			DialogUtil.dialog({
				title : '新建业务数据模版',
				content : __ctx
						+ "/platform/form/formDataTemplate/create.htm",
				area : [ '70%', '80%' ],
				btn : [{
							label : '下一步',
							iconCls : 'btn btn-success fa fa-arrow-circle-right',
							action : function(dialog, index) {
								var data = DialogUtil.getChildFrameWindow(index).formDataTemplateCreate.getNextData();
								if ($.isEmpty(data)) {
									return;
								}
								me.edit(data);
		...
		edit:function(params,id){
			var url =  __ctx+ "/platform/form/formDataTemplate/design.htm";
			if(id)
				url +="?id="+id;
			else 
				url  +="?formKey="+params.formKey;
			DialogUtil.dialog({
				params:params,
