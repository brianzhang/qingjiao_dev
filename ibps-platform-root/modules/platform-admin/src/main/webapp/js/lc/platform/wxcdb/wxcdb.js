...
(function() {
	...
	Resources.prototype = {
		...
		loadTree:function (){
			...
			var url=__ctx+"/platform/wxcdb/wxcdb/getTreeData.htm";
			$.post(url,{
				'systemId':me.systemId?me.systemId:2
			},function(result){
		...
		reFresh:function(){
			//刷新左边的树
			this.loadTree();
			var url=__ctx+"/platform/wxcdb/wxcdb/get.htm?id=";
		...
		treeOnLeftClick:function(me, treeNode) {
			...
			if(id== me._treeRootId)
				id ="";
			var url=__ctx+"/platform/wxcdb/wxcdb/get.htm?id="+id;
			$("#listFrame").attr("src",url);
		},
		...
		_exportNode : function(node){
			var url=__ctx +'/platform/wxcdb/wxcdb/exportXml.htm?id='+node.id+"&subSysId="+$("#subSystem").val();
		...
		_importNode : function(node){
			var me = this;
			var url=__ctx +'/platform/wxcdb/wxcdb/import.htm';
			param={
				id: node.id,
		...
		addNode:function(node){
			var url=__ctx+"/platform/wxcdb/wxcdb/edit.htm?parentId="+node.id+"&systemId="+node.systemId;
			$("#listFrame").attr("src",url);
		},
		editNode:function(node){
			var url=__ctx+"/platform/wxcdb/wxcdb/edit.htm?id="+node.id;
			$("#listFrame").attr("src",url);
		},
		/**
		 * 左侧菜单的滚动
		...
		delRes : function(me, node, rtn){
			var url =__ctx+"/platform/wxcdb/wxcdb/remove.htm";
			$.post(url,{
				id:node.id,
				cascade:rtn
				},
		...
		sortNode:function(node){
			...
			var url=__ctx +'/platform/wxcdb/wxcdb/sortList.htm?id='+node.id+'&systemId='+node.systemId;
			DialogUtil.dialog({
				title:'节点排序',
				area: ['50%', '55%'],
				content:url,
				 btn: [{
					   ...
					   action:function(dialog,index){
						  ...
						  if(data.length>0){
								var url=__ctx +'/platform/wxcdb/wxcdb/sortSave.htm';		
								var params = {ids:data};
		...
		moveNode:function(node){
			var me =this;
			var url=__ctx +'/platform/wxcdb/wxcdb/moveNode.htm?id='+node.id+'&systemId='+node.systemId;
			DialogUtil.dialog({
				title:'移动节点',
				area: ['50%', '55%'],
				...
				btn: [{
					   ...
					   action:function(dialog,index){
						...
						var url=__ctx +'/platform/wxcdb/wxcdb/saveMove.htm';		
