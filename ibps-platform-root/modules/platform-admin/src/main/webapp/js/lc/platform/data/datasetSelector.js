...
$(function() {
	...
	datasetSelector.init();
});


(function() {
	var template =
		'<span class="attach-span" name="dataset-span" data-id="#id#" data-key="#key#" data-name="#name#">' +
				'<span title="#name#">#name#</a>' +
				'&nbsp;' +
				'<a class="btn btn-ms" title="移除" href="javascript:datasetSelector.remove(\'#id#\',\'#key#\');"><i class="fa fa-remove"></i></a>' +
				'</span>'+
			'</span>';
	...
	DatasetSelector.prototype = {
		...
		init : function() {
			...
			this.gridUrl = params.gridUrl?params.gridUrl:(__ctx+ '/platform/data/dataset/listJson.htm');
			this.data = null;
		...
		_initContainer : function(){
			if(!this.isSingle){
				$("div[name='dataset_container']").height("80px");
			}
		},
		/**
		 * 初始列表
		...
		_initGridList : function() {
			...
			this.grid.GridList({
								...
								colNames: ['主键','名称','业务主键','类型','来源','是否树型'],
						        colModel: [{
						                 	   name:'id',
						                	   index: 'id_',
						                	 	hidden:true,
