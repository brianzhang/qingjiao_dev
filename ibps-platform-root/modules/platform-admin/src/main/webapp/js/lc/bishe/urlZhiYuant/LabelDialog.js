...
(function() {
	var defaults = {
			title:'标签选择框',
			url:__ctx+'/platform/org/partyEmployee/label.htm',
			isSingle:false, //是否单选
			type:1, //是否单选
			isObj:false//是否返回对象
	};
	 ...
	 * @returns
	 */
	LabelDialog= function(options){
		this.options = $.extend({}, defaults, options);
	};
	
	LabelDialog.prototype={
			/**
			 * 显示窗口
			 */
			show:function(){
					...
					area:['60%','80%'],
					 btn:[{
			                ...
			                action: function(dialog,index) {
			              	  var  data = DialogUtil.getChildFrameWindow(index).labelSelector.getData();
			              	  if($.isEmpty(data)){
			              	  ...
			              	  if(options.isObj){
			          			...
			          			$.each(data, function(i, content) {
									var labelId=content.id;
									/*var labelName=content.labelname;*/
									labelIds.push(labelId);
									/*labelNames.push(labelName);*/
								});
			          			options.callback(labelIds/*,labelNames*/);
			              	  }
			            ...
			            },
			            {
			                ...
			                action: function(dialog,index) {
			                	...
			                	$.each(data, function(i, content) {
			                		DialogUtil.getChildFrameWindow(index).labelSelector.remove(content.id,content.account);
			                	});
			                	DialogUtil.getChildFrameWindow(index).labelSelector.clearSelect();
			                	/*if(options.isObj){
				              		options.callback(data);
			                	}else{
			                		options.callback([], []);
