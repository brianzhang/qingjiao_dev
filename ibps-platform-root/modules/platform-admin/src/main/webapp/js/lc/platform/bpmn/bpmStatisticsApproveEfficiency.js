$(function() {
	bpmStatisticsApproveEfficiency = new BpmStatisticsApproveEfficiency();
	bpmStatisticsApproveEfficiency.init();
});
...
(function() {
	 ...
	 * @returns {ReportDef}
	 */
	BpmStatisticsApproveEfficiency = function() {
	 ...
	 * 方法
	 */
	BpmStatisticsApproveEfficiency.prototype = {
		consts : _consts,
		/**
		 * 初始化
		 */
		...
		_initFrameHeight : function() {
			...
			var newFrameHeight = $(window).height() - ofs - 45;
		...
		_initBts : function() {
			...
			$("#dateType").on("change",function(){
				var val = $(this).val();
				if(val=="year"){
					me.initDatepicker("yyyy");
				}else if(val=="month"){
					me.initDatepicker("yyyy-MM");
				}else if(val=="day"){
					me.initDatepicker("yyyy-MM-dd");
				}else{
					me.initDatepicker("yyyy-MM");
				}
 			});
		...
		initDatepicker: function(fmt){
			$("input[name='createTime']").remove();
			$("input[name='endTime']").remove();
			$("#createTime").append('<input type="text" name="createTime" datefmt="'+fmt+'"  class="form-control datepicker" />');
			$("#endTime").append('<input type="text" name="endTime" datefmt="'+fmt+'"  class="form-control datepicker" />');
		}
