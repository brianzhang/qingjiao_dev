...
(function() {
	...
	OfficeControl.prototype = {
		consts : {
			CAB_PATH 			: __ctx + "/media/office/OfficeControl.cab#version=5,0,3,9",
			CAB64_PATH 			: __ctx + "/media/office/OfficeControlx64.cab#version=5,0,3,9",
			...
			SECRET : 'D473E5BCF4704563EEDE919E47864735611402E6',
...
//			this.controlObjId 	= "TANGER_OCX";
		...
		getFirefoxChromeHtml : function(codeBase, clsid){
			...
			html += 'ForOnCustomMenuCmd2="CustomMenuCmd" _ProductCaption="哈尔滨工程大学" _ProductKey="D473E5BCF4704563EEDE919E47864735611402E6" '
			html += 'clsid="{' + clsid + '}">';
			html += '</object> ';
		...
		getIEHtml : function(codeBase, clsid){
			...
			html = '<object  id="'
					...
					+ '" classid="clsid:'
					+ clsid
					+ '" _Caption="轻教-OfficeOnline"'
					+'>';
			
			html += '<param name="ProductCaption" value="哈尔滨工程大学"><param name="ProductKey" value="' +  this.consts.SECRET +'">';
...
//					DialogUtil.warn("请使用ie浏览器进行保存");
//					return ;
//				}
				me.options.docName = me.options.fileName;
				me.saveFile();
				DialogUtil.closeAll();
				DialogUtil.msg('保存成功！');
				me.options.callback();
//				if("作业名称不需要加后缀" === me.options.docName){
//					DialogUtil.prompt({
//							formType: 0,
...
//				}
		...
		saveFile : function(cFileName, cType) {
			...
			var result 			= null,
				...
				fileName 		=encodeURI( docName+"."+docType),//文件名
			...
			if(this.options.extraParam)
				params = params + this.options.extraParam;
