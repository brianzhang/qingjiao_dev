 ...
 */
$(function() {
	customQuerySetting = new CustomQuerySetting();
	customQuerySetting.init();
});

(function() {
	// 定义常量
	var _consts = {
		SETTING : '#customQuerySetting'// 设置页面
	};
	/**
	 * 自定义对话框 对象
	 * 
	...
	CustomQuerySetting.prototype = {
		...
		_initData : function() {
			...
			if (fields.conditionField){
				fields.conditionField = (fields.conditionField).replace(/\s+/g,"");
				me.initConditionField(fields.conditionField);
			}
				

			if (fields.resultField){
				fields.resultField = (fields.resultField).replace(/\s+/g,"");
				me.initResultField(fields.resultField);
			}
		
		
			if (fields.sortField){
				fields.sortField = (fields.sortField).replace(/\s+/g,"");
		...
		_addSelect : function(obj){
			...
			trObj.append('<div class="margin-10">'+
					...
					'<input type="checkbox" '+
						...
						'type="radio" class="hover-pointer" name="opt-selected" '+
						'style="position: absolute; left: 5px; top: 6px;" /> <input '+
