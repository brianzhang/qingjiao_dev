...
(function() {
	//定义常量
	var 	_consts = {
			TABLEID : "#copyToNotify", // 表单对象
			NOTIFYTEMP:'copyToNotifyTemp', //抄送数据模板
			USERRULETEMP:'userRuleTemp' //人员配置数据模板
	};
	/**
	 * 流程定义 对象
	 * @returns {copyToNotify}
	 */
	CopyToNotify = function() {
		//定义属性
		this.copyToNotifyJson = [];
	...
	CopyToNotify.prototype = {
		 ...
		 * 添加或修改人员配置
		 * @param conf
		 * @param notify_i  必填，说明是copyToNotifyJson 中的第notify_i组抄送配置数据
		...
		_addUserRule:function(conf,notify_i,userRule_j){
			...
			if(userRule_j != undefined){
				param = me.copyToNotifyJson[notify_i].userAssignRules[userRule_j]
			}
			DialogUtil.dialog({
				   content: url,
				   params:param,
				   ...
				   btn: [{
					   ...
					   action:function(dialog,index){
						   ...
						   if(data){
								   if(data.groupNo==-1){
									   data.groupNo = me.copyToNotifyJson[notify_i].userAssignRules.length+1;
								   }
								   if( userRule_j != undefined){
									   me.copyToNotifyJson[notify_i].userAssignRules[userRule_j] = data;
								   }else{
									   me.copyToNotifyJson[notify_i].userAssignRules.push(data);
								   }
								   var userRuleHtml = me._renderHml({
									   userAssignRules : me.copyToNotifyJson[notify_i].userAssignRules,
									   notify_i : notify_i
		...
		_saveData : function(){
			...
			for(var i = 0; i<me.copyToNotifyJson.length; i++){
				var procNotify=me.copyToNotifyJson[i] , groupNo="" ;
		...
		_addNotify : function(){
			...
			me.copyToNotifyJson.push(newJson);
			me._initData();
		},
		 ...
		 * 修改到copyToNotifyJson中的第notify_i组的msgTypes属性中
		...
		_changeMsgType : function(conf,notify_i){
			...
			me.copyToNotifyJson[notify_i].msgTypes = msgTypeAry.join(",");
		...
		_delNotify : function(notify_i){
			...
			var me=this;
			if(me.copyToNotifyJson.length==1){
				DialogUtil.toastr("至少保留一个抄送配置!")
				return;
			}
        	DialogUtil.confirm( '确认删除吗？','提示信息', function(r) {
        		if (r) {
        			me.copyToNotifyJson.splice(notify_i,'1');
        			me._initData();
        		}
        	})
		},
		...
		_delUserRule : function(notify_i,userRule_j){
			...
			var me=this;
			me.copyToNotifyJson[notify_i].userAssignRules.splice(userRule_j,'1');
			me._initData();
		},
		
		_changeNo : function(conf,notify_i,userRule_j){
			var me = this;
			if(!me.copyToNotifyJson || !notify_i || !userRule_j) {
				return true;
			}
			var value = $(conf).val();
			me.copyToNotifyJson[notify_i].userAssignRules[userRule_j].groupNo=value;
		},
	};
