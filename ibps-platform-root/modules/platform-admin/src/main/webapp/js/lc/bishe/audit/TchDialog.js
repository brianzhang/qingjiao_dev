					...
					area:['60%','80%'],
					 btn:[{
			                ...
			                action: function(dialog,index) {
			              	  var  data = DialogUtil.getChildFrameWindow(index).tchSelector.getData();
			              	  if($.isEmpty(data)){
			            ...
			            },
			            {
			                ...
			                action: function(dialog,index) {
			                	...
			                	$.each(data, function(i, content) {
			                		DialogUtil.getChildFrameWindow(index).tchSelector.remove(content.id,content.account);
			                	});
			                	DialogUtil.getChildFrameWindow(index).stuSelector.clearSelect();
			                	/*if(options.isObj){
				              		options.callback(data);
			                	}else{
			                		options.callback([], []);
