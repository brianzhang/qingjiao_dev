 ...
 * <br>
	...
	参数说明：
	<p>	
		title【可选】 : 标题。类型：String，默认：'数据集选择器选择框'；
...
(function() {
	var defaults = {
			title:'数据集选择器',
			url:__ctx+'/platform/data/dataset/dialog.htm',
			isSingle:true //是否单选
	};
	 ...
	 * @returns
	 */
	DatasetDialog= function(options){
		this.options = $.extend({}, defaults, options);
	};
	
	DatasetDialog.prototype={
			/**
			 * 显示窗口
			 */
			show:function(){
					...
					area:['85%','600px'],
					 btn:[{
			                ...
			                action: function(dialog,index) {
			              	  var  data = DialogUtil.getChildFrameWindow(index).datasetSelector.getData();
			              	  if($.isEmpty(data)){
			              		  DialogUtil.toastr("请选择数据集！");
			              		  return;
			              	  }
