	...
	RoleDialog.prototype = {
		...
		_init : function() {
			...
			if(data.initData){
				...
				if(source == 'spec'){
	  				$('input[name="roleKey"]').val(jsonData["roleKey"]);
	  				$('textarea[name="roleName"]').val(jsonData["roleName"]);
	  			}else if(source == 'node'){
		...
		_choiceUser:function(conf){
			var params =[],
			keyStr = $("input[name='roleKey']").val(),
			nameStr =  $("textarea[name='roleName']").val();
			if(!$.isEmpty(keyStr)){
				var keyAry = keyStr.split(',');
				var nameAry = nameStr.split(',');
				for(var i =0; i<keyAry.length;i++){
					var paramObj = {roleAlias:keyAry[i],name:nameAry[i]};
			...
			new PartyRoleDialog({
				...
				callback:function(data){
					...
					$("input[name='roleKey']").val(keys); 
					$("textarea[name='roleName']").val(names);
				}
			}).show();
		},
 ...
 */
 function getData(){
	...
	if(source == 'spec'){
		retultData["roleKey"] = $('input[name="roleKey"]').val();
		retultData["roleName"] = $('textarea[name="roleName"]').val();
		sourceText = sourceText+"["+retultData["roleName"]+"]";
		if(!retultData["roleKey"] || '' == retultData["roleKey"]){
			 DialogUtil.toastr("请指定角色"); 
			 return false;
		}
	}else if(source == 'node'){
