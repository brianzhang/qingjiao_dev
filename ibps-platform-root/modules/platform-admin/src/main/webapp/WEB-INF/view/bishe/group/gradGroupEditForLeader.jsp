...
<%-- 			 	<div class="fr_response_field col-sm-12" >
 			 ...
 			 <div class="fr_response_field col-sm-12" >
				...
				<div class="input-icon" >
					<i class="fa fa-calendar"></i>
					<input type="text" readonly="readonly" class="fr-form-control datepicker" datefmt="yyyy-MM-dd"   name="m:gradGroup:date"   value="<fmt:formatDate value="${gradGroup.date}"  pattern="yyyy-MM-dd"/>" validate="{required:false}"/>
				</div>
				 	</div>
			  	</div>
			</div> 
				...
				<input type="text" class="fr-form-control" name="m:gradGroup:place" value="${gradGroup.place}" validate="{required:false}"/>
			 	...
			 	<div class="fr_response_field col-sm-12" >
			 	<div class="fr-form-group"> 
				 	<label class="fr-control-label">${gradGroup.type}名称</label>
				  	<div class="fr-form-block">
...
<%-- 			<div class="fr_response_field col-sm-12" >
						...
						<select name="Q^TERM^S" class="form-control search-select" onchange="cg()">
							<option value="-1"></option>	
								<c:forEach items="${typeList}" var="type">
									<option value="${type}" <c:if test="${type == curtype }">selected</c:if>>${type}</option>
							   </c:forEach>
						</select>
				 	</div>
			  	</div>
			...
			<div class="fr_response_field col-sm-12" >
			 	<div class="fr-form-group"> 
				 	<label class="fr-control-label">类型</label>
				  	<div class="fr-form-block">
				<select class="fr-form-control" name="m:gradGroup:type"   validate="{required:false}">
					<option value="-1"></option>	
					<c:forEach items="${typeList}" var="type">
						<option value="${type}" <c:if test="${type == curtype }">selected</c:if>>${type}</option>
					</c:forEach>
 			 ...
 			 <div class="fr_response_field col-sm-12" >
			 	<div class="fr-form-group"> 
				 	<label class="fr-control-label">${gradGroup.type}组长</label>
				  	<div class="fr-form-block">
				<input type="text" class="fr-form-control" name="m:gradGroup:leader" value="${gradGroup.leader}" validate="{required:false}"/>
				 	</div>
			  	</div>
			</div> 
			 ...
			 <div class="fr_response_field col-sm-12" >
			 	<div class="fr-form-group"> 
				 	<label class="fr-control-label">${gradGroup.type}组员</label>
				  	<div class="fr-form-block">
				<input type="text" class="fr-form-control" name="m:gradGroup:leader" value="" validate="{required:false}"/>
				 	</div>
			  	</div>
			</div> 
<%-- 			 	<div class="fr_response_field col-sm-12" >
