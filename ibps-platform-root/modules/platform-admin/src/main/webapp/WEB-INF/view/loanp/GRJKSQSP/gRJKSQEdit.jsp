...
<script type="text/javascript"> 
 function Choose(){
	...
	var choose = $("#select11 option:selected");
	//alert($("#select11 option:selected").val());
 	if(choose.val() =='1'){
 	//	alert("test"+$("#sfid").val());
 ...
 function ChoosePoxx(){
  ...
  function checkPostcode(){
    ...
    function checkPone(){
	  var phone = window.document.getElementById("phonecode");
	   var  phonecode = phone.value;
	   var  path1 = /^1(3|4|5|7|8)\d{9}$/;
	   if(!(path1.test(phonecode))){ 
		   DialogUtil.msg("手机号格式错误，请重新填写",
					function(rtn) {
					}); 
	    }
  }
    ...
    function checkNumber(){
  	  var  lianxi = window.document.getElementById("lianxi");
		...
		<div class="wrapper wrapper-content  animated fadeInRight col-sm-12">
			...
			<div class="">
				...
				<select class="fr-form-control"  id ="select1" name="m:gRJKSQ:sfgtgsh"  value="${gRJKSQ.sfgtgsh}" validate="{required:true}" onchange="Choose();">
				    ...
				    <option value="农户" <c:if test="${gRJKSQ.khlx=='农户'}">selected="selected"</c:if>>农户</option>
				    <option value="非农户" <c:if test="${gRJKSQ.khlx=='非农户'}">selected="selected"</c:if>>非农户</option>
				</select>
				 	</div>
			  	</div>
			</div>
				...
				<select class="fr-form-control" name="m:gRJKSQ:sfyhjzm"  value="${gRJKSQ.sfyhjzm}" validate="{required:true}">
				    ...
				    <option value="否" <c:if test="${gRJKSQ.sfyhjzm=='否'}">selected="selected"</c:if>>否</option>
				    <option value="是" <c:if test="${gRJKSQ.sfyhjzm=='是'}">selected="selected"</c:if>>是</option>
				</select>
				 	</div>
			  	</div>
			</div>
			 	...
			 	<div class="fr-form-group"> 
				 	<label class="fr-control-label">民族 *</label>
				  	<div class="fr-form-block">
				<input  type="text"  class="fr-form-control" name="m:gRJKSQ:mz"  value="${gRJKSQ.mz}" validate="{required:true}"/>
				 	</div>
			  	</div>
			</div>
				...
				<select class="fr-form-control" name="m:gRJKSQ:khxb"  value="${gRJKSQ.khxb}" validate="{required:true}">
				    ...
				    <option value="男" <c:if test="${gRJKSQ.khxb=='男'}">selected="selected"</c:if>>男</option>
				    <option value="女" <c:if test="${gRJKSQ.khxb=='女'}">selected="selected"</c:if>>女</option>
				</select>
				 	</div>
			  	</div>
			</div>
				...
				<select class="fr-form-control" name="m:gRJKSQ:zjlb"  value="${gRJKSQ.zjlb}" validate="{required:true}">
					 <option value="">请选择</option>
				    <option value="身份证" <c:if test="${gRJKSQ.zjlb=='身份证'}">selected="selected"</c:if>>身份证</option>
				    <option value="军官证" <c:if test="${gRJKSQ.zjlb=='军官证'}">selected="selected"</c:if>>军官证</option>
				    <option value="护照" <c:if test="${gRJKSQ.zjlb=='护照'}">selected="selected"</c:if>>护照</option>
				</select>
				 	</div>
			  	</div>
			</div>
				...
				<select class="fr-form-control" name="m:gRJKSQ:sfbxgd"  value="${gRJKSQ.sfbxgd}" validate="{required:true}">
					<option value="否" <c:if test="${gRJKSQ.sfbxgd=='否'}">selected="selected"</c:if>>否</option>
				    <option value="是" <c:if test="${gRJKSQ.sfbxgd=='是'}">selected="selected"</c:if>>是</option> 
				</select>
				 	</div>
			  	</div>
			</div>
			 	...
			 	<div class="fr-form-group"> 
				 	<label class="fr-control-label">通信地址邮政编码 *</label>
				  	<div class="fr-form-block">
				<input type="text" class="fr-form-control" id="postcode"  name="m:gRJKSQ:txdzyzbm" value="${gRJKSQ.txdzyzbm}"  validate="{required:true}" onblur="checkPostcode();" />
				 	</div>
			  	</div>
			</div>
			 	<div class="fr_response_field col-sm-3" >
			 	<div class="fr-form-group"> 
				 	<label class="fr-control-label">手机号码 *</label>
				  	<div class="fr-form-block">
				<input type="text" class="fr-form-control" id ="phonecode"  name="m:gRJKSQ:sjhm" value="${gRJKSQ.sjhm}" validate="{required:true}"  onblur = "checkPone();"/>
				 	</div>
			  	</div>
			</div>
			 	<div class="fr_response_field col-sm-3" >
			 	<div class="fr-form-group"> 
				 	<label class="fr-control-label">其他联系方式</label>
				  	<div class="fr-form-block">
				<input type="text" class="fr-form-control"  id="lianxi" name="m:gRJKSQ:qtlxfs"  value="${gRJKSQ.qtlxfs}"  onblur ="checkNumber();"/>
				 	</div>
			  	</div>
			</div>
			 	<div class="fr_response_field col-sm-3" >
				...
				<select class="fr-form-control" name="m:gRJKSQ:jzzt"  value="${gRJKSQ.jzzt}" validate="{required:true}">
				    ...
				    <option value="2" <c:if test="${gRJKSQ.jzzt=='2'}">selected="selected"</c:if>>临时</option>
				</select>
				 	</div>
			  	</div>
			</div>
				...
				<input type="text" class="fr-form-control" name="m:gRJKSQ:qtjjly" value="${gRJKSQ.qtjjly}" validate="{required:true"/>
			 	...
			 	<div class="fr-form-group"> 
				 	<label class="fr-control-label">个人综合年收入</label>
				  	<div class="fr-form-block">
				<input type="text" class="fr-form-control"  id="person"  name="m:gRJKSQ:grzhnsr" value="${gRJKSQ.grzhnsr}" onblur="checkMoneys('person');"/>
				 	</div>
			  	</div>
			</div>
			 	<div class="fr_response_field col-sm-3" >
			 	<div class="fr-form-group"> 
				 	<label class="fr-control-label">家庭人均年收入</label>
				  	<div class="fr-form-block">
				<input type="text" class="fr-form-control" id="familyPerson" name="m:gRJKSQ:jtrjnsr" value="${gRJKSQ.jtrjnsr}"  onblur="checkMoneys('familyPerson');"/>
				 	</div>
			  	</div>
			</div>
			 	<div class="fr_response_field col-sm-3" >
			 	<div class="fr-form-group"> 
				 	<label class="fr-control-label">家庭年均支出</label>
				  	<div class="fr-form-block">
				<input type="text" class="fr-form-control" id="familyAvg" name="m:gRJKSQ:jtnjzc" value="${gRJKSQ.jtnjzc}" onblur="checkMoneys('familyAvg');"/>
				 	</div>
			  	</div>
			</div>
			 	<div class="fr_response_field col-sm-3" >
				...
				<select class="fr-form-control" name="m:gRJKSQ:ywsgx"  value="${gRJKSQ.ywsgx}" validate="{required:true}">
			 	...
			 	<div class="fr-form-group"> 
				 	<label class="fr-control-label">是否个体工商户*</label>
				  	<div class="fr-form-block">
				<select class="fr-form-control"  id ="select11" name="m:gRJKSQ:sfgtgsh"  value="${gRJKSQ.sfgtgsh}" validate="{required:true}" onchange="Choose();">
					 <option value="">请选择</option>
