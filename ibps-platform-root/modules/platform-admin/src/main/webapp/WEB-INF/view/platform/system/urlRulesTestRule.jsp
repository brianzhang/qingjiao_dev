				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<textarea class="form-control" id="script" name="script"
							style="width: 100%; height: 220px;">${fn:escapeXml(sysUrlRules.script)}11</textarea>
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-10">
						<select name="enable"  id="enable" class="inputText input-wh-1">
							<option value="0"
								<c:if test="${sysUrlRules.enable eq 0}"> selected="selected"</c:if>>否</option>
							<option value="1"
								<c:if test="${sysUrlRules.enable eq 1}"> selected="selected"</c:if>>是</option>
						</select>
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-10">
						<input type="text" class="form-control" id="descp" name="descp"
							value="${sysUrlRules.descp}" validate="{required:false}" />
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-10">
							...
							id="sort" name="sort"
							value="${sysUrlRules.sort}"
							validate="{required:false,number:true,maxIntLen:10}" />
					</div>
				</div>
				<input type="hidden" name="id" value="${sysUrlRules.id}" />
				<input type="hidden" name="sysUrlId" id="sysUrlId" value="${sysUrlRules.sysUrlId}" />
			</form>
		</div>
	</div>
</body>
