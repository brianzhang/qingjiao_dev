...
public class Json2Po {
	
	public static AbstractParam parse(JSONObject jo,Class T){
		try {
			if(!(T.newInstance() instanceof AbstractParam))
				return null;
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		AbstractParam res = null;
		try {
			res = (AbstractParam)T.newInstance();
			setFieldValue(res, kv);
	...
	public static List parse(JSONArray ja,Class T){
...
//              String fieldKeyName = parKeyName(field.getName());  
     ...
     * @return 去掉开头的get 
     */  
    private String parKeyName(String fieldName) {  
